#[cfg(test)]
mod delete_watching_tests {
    use backlog_api_core::IntoRequest;
    use backlog_core::identifier::{Identifier, WatchingId};

    use crate::api::delete_watching::DeleteWatchingParams;

    #[test]
    fn test_params_creation() {
        let params = DeleteWatchingParams::new(WatchingId::from(123));
        assert_eq!(params.watching_id.value(), 123);
    }

    #[test]
    fn test_path_generation() {
        let params = DeleteWatchingParams::new(WatchingId::from(456));
        assert_eq!(params.path(), "/api/v2/watchings/456");
    }

    #[test]
    fn test_no_query_params() {
        let params = DeleteWatchingParams::new(WatchingId::from(789));
