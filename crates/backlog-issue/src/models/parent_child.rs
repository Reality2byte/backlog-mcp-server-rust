...
impl ParentChildCondition {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            ParentChildCondition::NeitherParentIssueNorChildIssue => "Neither Parent nor Child Issues",
...
mod tests {
    ...
    fn test_parent_child_condition_values() {
        ...
        assert_eq!(ParentChildCondition::NeitherParentIssueNorChildIssue as u8, 3);
        assert_eq!(ParentChildCondition::ParentIssue as u8, 4);
    }

    #[test]
    fn test_serialization() {
        assert_eq!(serde_json::to_string(&ParentChildCondition::All).unwrap(), "0");
        assert_eq!(serde_json::to_string(&ParentChildCondition::ChildIssue).unwrap(), "2");
        assert_eq!(serde_json::to_string(&ParentChildCondition::ParentIssue).unwrap(), "4");
    }

    #[test]
    fn test_deserialization() {
        assert_eq!(serde_json::from_str::<ParentChildCondition>("0").unwrap(), ParentChildCondition::All);
        assert_eq!(serde_json::from_str::<ParentChildCondition>("2").unwrap(), ParentChildCondition::ChildIssue);
        assert_eq!(serde_json::from_str::<ParentChildCondition>("4").unwrap(), ParentChildCondition::ParentIssue);
    }

    #[test]
    fn test_description() {
        assert_eq!(ParentChildCondition::All.description(), "All issues (no filtering by parent/child status)");
        assert_eq!(ParentChildCondition::ExcludeChildIssue.description(), "Exclude Child Issues");
        assert_eq!(ParentChildCondition::ChildIssue.description(), "Child Issues only");
        assert_eq!(ParentChildCondition::NeitherParentIssueNorChildIssue.description(), "Neither Parent nor Child Issues");
        assert_eq!(ParentChildCondition::ParentIssue.description(), "Parent Issues only");
    }
    ...
    fn test_display() {
        ...
        assert_eq!(ParentChildCondition::NeitherParentIssueNorChildIssue.to_string(), "3");
