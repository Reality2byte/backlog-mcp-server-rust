...
/// Parameters for getting the attachment list of an issue.
...
mod tests {
    ...
    #[test]
    fn test_get_attachment_list_params_new() {
        let issue_id_or_key = IssueIdOrKey::Key(IssueKey::from_str("TEST-123").unwrap());
        let params = GetAttachmentListParams::new(issue_id_or_key.clone());
        assert_eq!(params.issue_id_or_key, issue_id_or_key);
    }

    #[test]
    fn test_get_attachment_list_params_into_request() {
        let issue_id_or_key = IssueIdOrKey::Key(IssueKey::from_str("TEST-123").unwrap());
        let params = GetAttachmentListParams::new(issue_id_or_key);

        assert_eq!(params.method(), HttpMethod::Get);
        assert_eq!(params.path(), "/api/v2/issues/TEST-123/attachments");
    }
}
